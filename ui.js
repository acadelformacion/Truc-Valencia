...
// Variable para que el marcador recuerde los puntos anteriores
let _oldHUD = { 0: 0, 1: 0 }; 
...
async function animateHUDPoints(id, targetValue, hudIdx) {
  ...
  // Si los puntos bajan (nueva partida) o son iguales, pintamos sin animar
  ...
  // Bucle para subir de uno en uno
  while (current < targetValue) {
    current++;
    _oldHUD[hudIdx] = current;
    
    // Esperamos un poco entre cada punto (0.6 segundos)
    await new Promise(r => setTimeout(r, 600)); 

    el.textContent = current;
    
    // Lanzamos la animación visual de "pop"
...
function renderHUD(state){
  ...
  // --- Lógica de Animación del Marcador ---
  if (state.hand?.status === 'finished') {
    // Si la mano ha terminado, animamos la subida
    animateHUDPoints('hudScore0', sMy, 0);
    animateHUDPoints('hudScore1', sRiv, 1);
  } else {
    // Si estamos jugando o empezando, ponemos el número directo
    $('hudScore0').textContent = String(sMy);
    $('hudScore1').textContent = String(sRiv);
    _oldHUD[0] = sMy;
    _oldHUD[1] = sRiv;
  }
  // ----------------------------------------
